 ...
 */
Dstar::Dstar(int maxSteps, double C1){

  this->maxSteps = maxSteps;  // node expansions before we give up
  this->C1       = C1;      // cost of an unseen cell
 ...
 * [S. Koenig, 2002]
 */
void Dstar::init(double sX, double sY, double gX, double gY) {
  ...
  s_start.x = std::round(sX);
  s_start.y = std::round(sY);
  s_goal.x  = std::round(gX);
  s_goal.y  = std::round(gY);
 ...
 * As per [S. Koenig, 2002]
 */
void Dstar::updateCell(double x, double y, double val) {
 ...
 */
void Dstar::updateStart(double x, double y) {

  s_start.x = std::round(x);
  s_start.y = std::round(y);
 ...
 */
void Dstar::updateGoal(double x, double y) {
  ...
  s_goal.x  = std::round(x);
  s_goal.y  = std::round(y);
