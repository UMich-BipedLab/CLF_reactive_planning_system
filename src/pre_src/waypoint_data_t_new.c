...
void pack_waypoint_data_t(const waypoint_data_t *bus, unsigned char bytes[216])
{
  ...
  float b_x[40];
  for (i0 = 0; i0 < 7; i0++) {
    x[i0] = (float)bus->controlsignal[i0];
  }
  ...
  memcpy((void *)&b_y[0], (void *)&x[0], (unsigned int)((size_t)28 * sizeof
          (unsigned char)));
  for (i0 = 0; i0 < 40; i0++) {
    b_x[i0] = (float)bus->waypoints[i0];
  }

  memcpy((void *)&c_y[0], (void *)&b_x[0], (unsigned int)((size_t)160 * sizeof
          (unsigned char)));
  for (i0 = 0; i0 < 28; i0++) {
    bytes[i0] = y[i0];
    bytes[i0 + 28] = b_y[i0];
  ...
  memcpy(&bytes[56], &c_y[0], 160U * sizeof(unsigned char));
}

void unpack_waypoint_data_t(const unsigned char bytes[216], waypoint_data_t *bus)
{
  int i;
  float y[7];
  unsigned char x[28];
  ...
  memcpy(&b_x[0], &bytes[56], 160U * sizeof(unsigned char));
